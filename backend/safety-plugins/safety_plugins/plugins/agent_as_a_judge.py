...
class LlmAsAJudge(BasePlugin):
    ...
    def __init__(
        ...
        judge_on: set[JudgeOn] = {JudgeOn.USER_MESSAGE, JudgeOn.TOOL_OUTPUT},
    ) -> None:
    ...
    def _sanitize_for_judge(self, data: dict[str, Any]) -> str:
        """Sanitizes sensitive data from dictionaries before logging or sending to judge.
        
        Redacts common sensitive keys like passwords, tokens, API keys, etc.
        ...
        def redact_dict(obj: Any) -> Any:
            ...
            if isinstance(obj, dict):
                ...
                return tuple(redact_dict(item) for item in obj)
            else:
                return obj
    ...
    async def _is_unsafe(self, message: str) -> bool:
        ...
        # Log sanitized version to avoid exposing PII
        logging.debug("[%s]: Judge analysis completed (is_unsafe: %s)", author, is_unsafe)
        return is_unsafe
    ...
    async def before_tool_callback(
        ...
        if JudgeOn.BEFORE_TOOL_CALL not in self._judge_on:
            return None
        # Sanitize tool_args to avoid exposing PII/credentials
        sanitized_args = self._sanitize_for_judge(tool_args)
        message = f"<tool_call>\nTool call: {tool.name}({sanitized_args})\n</tool_call>"
        if await self._is_unsafe(message):
    ...
    async def after_tool_callback(
        ...
        if JudgeOn.TOOL_OUTPUT not in self._judge_on:
            return None
        # Sanitize result to avoid exposing PII/credentials
        sanitized_result = self._sanitize_for_judge(result)
