...
export default function App() {
  ...
  const retryWithBackoff = async (
    ...
    let lastError: Error = new Error('Retry failed: no attempts made');
  ...
  const createSession = async (): Promise<{ userId: string, sessionId: string, appName: string }> => {
    ...
    // Get userId from environment variable or generate a new one
    const userId = import.meta.env.VITE_USER_ID || `u_${uuidv4()}`;

    const response = await fetch(`/api/apps/app/users/${userId}/sessions/${generatedSessionId}`, {
      method: "POST",
      headers: {
  ...
  const handleCancel = useCallback(() => {
    ...
    setWebsiteCount(0);
    // State reset without page reload - React-friendly approach
  }, []);
